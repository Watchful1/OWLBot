...
teamMatches = {}
for stage in stages:
	log.debug("Stage: "+str(stage['name']))
	...
	for match in stage['matches']:
		...
		game = {'home': match['competitors'][0]['abbreviatedName'],
				...
				'date': datetime.datetime.strptime(match['startDate'], "%Y-%m-%dT%H:%M:%S.000Z").replace(tzinfo=timezone.utc),
				'stage': stageName
				}
		matches.append(game)
		...
		log.debug("Match: "+game['home']+" vs "+game['away']+" at "+game['date'].astimezone(timezones['EST']).strftime("%m/%d %I:%M"))
